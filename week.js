...
function seriesDurationsMinutes(episode) {
  return episode.days * 24 * 60 + episode.hours * 60 + episode.minutes;
}
...
function logOutEpisode() {
  let totalLifePercents = 0;

  seriesDurations.forEach((episode) => {
    const durationMinutes = seriesDurationsMinutes(episode);
    const lifePercents = calculationLifePercents(durationMinutes);

    totalLifePercents += lifePercents;

    console.log(
      `${episode.title} took ${lifePercents.toFixed(4)}% of my life.`
    );
  });
  console.log(
